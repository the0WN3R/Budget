...
function getSupabaseClient() {
  if (!supabaseClient) {
    if (!createClientFunc) {
      ...
      // Try CommonJS build first, fall back to main entry
      try {
        // Try to require the CommonJS build directly
        let supabaseModule
        try {
          supabaseModule = require('@supabase/supabase-js/dist/cjs/index.js')
        } catch (cjsError) {
          // Fall back to main entry (might be ESM but webpack might have bundled it)
