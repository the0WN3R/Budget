  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        // Response is HTML or empty
        const text = await response.text()
        throw new Error(`API Error (${response.status}): ${text || response.statusText}. Please check if the API endpoint exists and environment variables are configured.`)
      }
