...
async function apiRequest(endpoint, options = {}) {
  ...
  // Build config object - method must be explicitly set
  ...
  // Spread other options (but method is already set, so it won't be overridden)
  ...
  // Log the final config method for debugging
  if (typeof window !== 'undefined') {
    console.log('[API Client] Final request config method:', config.method)
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Create a helpful error message for specific status codes
      if (response.status === 405) {
        throw new Error(`Method not allowed (405). The endpoint may not support ${config.method || 'GET'} requests.`)
      }
      
      // For 409 (Conflict), just return the message directly without status code prefix
      if (response.status === 409) {
        throw new Error(errorMessage)
      }
      
      // For 401 (Unauthorized) on login/signup endpoints, show user-friendly message
    ...
    if (error instanceof TypeError) {
      ...
      if (
        ...
        throw new Error('Internet connection error. Please try again later.')
      }
    }
...
export const budgetAPI = {
  ...
  tabs: {
    ...
    async getAll(budgetId) {
      ...
      return apiRequest(`/api/budgets/${budgetId}/tabs`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
        },
     ...
     * Update a tab
     */
    async update(budgetId, tabId, updates) {
      const token = getSession()?.access_token
      if (!token) {
        throw new Error('Not authenticated')
      }

      return apiRequest(`/api/budgets/${budgetId}/tabs/${tabId}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  expenses: {
    ...
    async getAll(budgetId) {
      ...
      return apiRequest(`/api/budgets/${budgetId}/expenses`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
        },
     ...
     */
    async create(budgetId, tabId, amount, description = null, expenseDate = null) {
      const token = getSession()?.access_token
      if (!token) {
        throw new Error('Not authenticated')
      }
...
export const supportAPI = {
   ...
   * Contact support
   */
  async contact(subject, message, email = null) {
    const token = getSession()?.access_token
