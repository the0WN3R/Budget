...
async function apiRequest(endpoint, options = {}) {
  ...
  const config = {
    method: options.method || 'GET', // Set method first
    headers: {
      'Content-Type': 'application/json',
      ...options.headers,
    },
    ...options, // Spread options after (method will be overridden if in options, which is what we want)
  }
  
  // Ensure method is explicitly set (in case options.method was undefined)
  if (!config.method) {
    config.method = 'GET'
  }
